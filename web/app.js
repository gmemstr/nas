...
function getFileListing(provider, path = "") {
  ...
  fetch(`/api/files/${provider}${path}`)
    ...
    .then((data) => {
      ...
      <div class="list">
        ${files.map(file =>
          `<div class="item"><a class="${file.is_dir == "true" ? "directory" : "file"}" href="${file.is_dir != "true" ? `/api/files/${provider}${path}/${file.name}` : `#${provider}${path === "" ? "" : path}/${file.name}`}">
            <span>${file.is_dir ? '<img src="/folder.svg"/>' : '<img src="/file.svg"/>'}${file.name}</span>
          </a><button onclick="deleteFile('${provider}', '${path === "" ? '' : path}', '${file.name}')"><img src="/trash.svg"/></button></div>
          `
        ).join('')}
      </div>
      `
